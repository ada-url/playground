...
function getDiagram(props: Props) {
    ...
    let lines: string[][] = [
        props.href.split(''),
        new Array(href.length).fill(' ')
    ]

    let omittable_values = [components.hash_start, components.search_start]

    for (let value of omittable_values) {
        if (value !== omitted) {
            lines[1][value] = '|'
        }
    }

    let non_omittable_values = [
    ...
    for (let value of non_omittable_values) {
        if (value !== href.length) {
            lines[1][value] = '|'
        }
    }

    let line2 = lines[1].slice()
    ...
    let line4 = lines[1].slice()
    if (components.pathname_start != href.length) {
    ...
    let line5 = lines[1].slice()
    if (components.host_end != href.length) {
    ...
    let line6 = lines[1].slice()
    if (components.host_start != href.length) {
    ...
    let line7: string[] = lines[1].slice()
    if (components.username_end != href.length) {
    ...
    let line8 = lines[1].slice()
    if (components.protocol_end != href.length) {
...
export default function ParsingResult(props: Props) {
    const alert = (
        <Alert>
            <Terminal className="h-4 w-4"/>
            <AlertTitle>{props.result === 'success' ? 'Parsing successful!' : 'Parsing failed!'}</AlertTitle>
            <AlertDescription>
                {
